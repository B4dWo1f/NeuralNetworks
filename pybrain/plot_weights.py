...
fol = '../../Dropbox/Diana_summer_INL/src/samples/lutchin_wire_matrix/4/the_hybrid'
...
fig, ax = plt.subplots()
ax.scatter(X,Y,s=50,edgecolors='none',zorder=10)


wmax = np.max([np.max(np.abs(w)) for w in Ws])
links = []
for il in range(1,len(layers)):
   ...
   for i in range(len(l1)):
      for j in range(len(l2)):
         ...
         x = [l1[i][0],l2[j][0]]
         y = [l1[i][1],l2[j][1]]
         ax.plot(x,y,'k-',lw=5*abs(w[i,j]/wmax),zorder=0)
plt.show()
